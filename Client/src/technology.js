...
const TechnologyBlogPage = () => {
  ...
  useEffect(() => {
    fetchTechnologyBlogPosts();
  }, []);

  const fetchTechnologyBlogPosts = async () => {
    try {
      const response = await BlogService.getBlogsByCategory("technology");
      setPosts(response.data);
      setLoading(false);
    } catch (error) {
      console.error("Error fetching technology blog posts:", error);
      setError("Failed to load technology blogs.");
      setLoading(false);
    }
  };
  ...
  return (
    <div style={pageContainer}>
      <header style={headerStyle}>
        <h1 style={titleStyle}>💻 Technology Blog</h1>
      </header>
      ...
      {loading ? (
        ...
        <div style={blogContainer}>
          {posts.length > 0 ? (
            ...
            <p style={noPostsMessage}>No technology blog posts available.</p>
          )}
        </div>
      )}
    </div>
