...
const TravelBlogPage = () => {
  ...
  const fetchTravelBlogPosts = async () => {
    try {
      const response = await BlogService.getBlogsByCategory("travel");
      setPosts(response.data);
      setLoading(false);
    } catch (error) {
      console.error("Error fetching travel blog posts:", error);
      setError("Failed to load travel blogs.");
      setLoading(false);
    }
  };
  ...
  return (
    <div style={pageContainer}>
      <header style={headerStyle}>
        <h1 style={titleStyle}>✈️ Travel Blog</h1>
      </header>
      ...
      {loading ? (
        ...
        <div style={blogContainer}>
          {posts.length > 0 ? (
            ...
            <p style={noPostsMessage}>No travel blog posts available.</p>
          )}
        </div>
      )}
    </div>
...
//       setCommentName("");
//       setCommentContent("");
//       setShowCommentForm(false); // Optionally hide form
//     } catch (error) {
//       console.error("Error posting comment:", error);
//     }
//   };
