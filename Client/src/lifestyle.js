...
const LifestyleBlogPage = () => {
  ...
  useEffect(() => {
    fetchLifestyleBlogPosts();
  }, []);

  const fetchLifestyleBlogPosts = async () => {
    try {
      const response = await BlogService.getBlogsByCategory("lifestyle");
      setPosts(response.data);
      setLoading(false);
    } catch (error) {
      setError("Failed to load lifestyle blog posts.");
      setLoading(false);
    }
  };
  ...
  const handleCommentSubmit = async (postId) => {
    ...
    try {
      ...
      fetchComments(postId); // refresh after posting
      setCommentName("");
      setCommentContent("");
  ...
  return (
    <div style={pageContainer}>
      <header style={headerStyle}>
        <h1 style={titleStyle}>🌿 Lifestyle Blog</h1>
      </header>
      ...
      {loading ? (
        ...
        <div style={blogContainer}>
          {posts.length > 0 ? (
            ...
            <p style={noPostsMessage}>No lifestyle blog posts available.</p>
          )}
        </div>
      )}
    </div>
...
// Styles (adjusted for single blog post per row)
const pageContainer = {
...
const blogContainer = {
  display: 'grid',
  gridTemplateColumns: '1fr', // Single blog per row
  gap: '20px',
};
