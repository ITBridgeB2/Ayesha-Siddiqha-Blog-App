...
const BooksBlogPage = () => {
  ...
  const filter = new Filter(); // ✅ Create an instance

  useEffect(() => {
    fetchBooksBlogPosts();
  }, []);

  const fetchBooksBlogPosts = async () => {
    try {
      const response = await BlogService.getBlogsByCategory("books");
      setPosts(response.data);
      setLoading(false);
    } catch (error) {
      console.error("Error fetching books blog posts:", error);
      setError("Failed to load books blogs.");
      setLoading(false);
    }
  };
  ...
  const handleCommentSubmit = async (postId) => {
    ...
    // ✅ Check for bad words
    if (filter.isProfane(commentName) || filter.isProfane(commentContent)) {
  ...
  return (
    <div style={pageContainer}>
      <header style={headerStyle}>
        <h1 style={titleStyle}>📚 Books Blog</h1>
      </header>
      ...
      {loading ? (
        ...
        <div style={blogContainer}>
          {posts.length > 0 ? (
            ...
            <p style={noPostsMessage}>No books blog posts available.</p>
          )}
        </div>
      )}
    </div>
...
// // Styles (same as TravelBlogPage, extended for comments section)
// const pageContainer = {
//   fontFamily: 'Arial, sans-serif',
//   background: '#f0f8ff',
//   padding: '20px',
...
//                 <h2 style={postTitleStyle}>{post.title}</h2>
//                 <p style={postDescriptionStyle}>{post.content || post.description}</p>
//                 <button style={readMoreButtonStyle}>Read More</button>
//               </div>
//             ))
//           ) : (
//             <p style={noPostsMessage}>No books blog posts available.</p>
